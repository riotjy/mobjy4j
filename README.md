...
**mobjy** for Java is a tool that accepts a data model specification in YAML, and outputs Java and C++ embeddable code comprised of
...
**mobjy** automates the generation of ample Java and C++ code using a fairly easy to write YAML specification. (see YAML example below)

The idea for **mobjy** came from hardships encountered in past projects where data objects and handlers were written in multiple languages, 
and each data model change required the programmers to revise and modify multiple data objects and converters code in multiple projects and languages. 

**mobjy** supports object oriented hierarchies (inheritance) making the conversion between in process data objects and JSON data possible while
ensuring the proper creation of the intended in process class instances.

In its current verison **mobjy** is using the *gson* and *nlohmann/json* for parsing and extracting JSON data.
It avoids using the automatic object creation provided by these libraries in order to support library interchangeability in the future (e.g. *jackson*, *jsoncpp*).

**mobjy** is intended to be extended to support other languages (golang, python, etc...) and also multiple approaches for the same language.
For example in addition the the C++11 compatible code currently generated by the tool, future C++ code may include optimizatoins for C++14/17/20.
This depend on available time and resources to continue development.
...
public class BaseClass {
  ...
  protected ArrayList<Integer> dblArrTwo = new ArrayList<>();

  public int sizeDblArrTwo() {
    return this.dblArrTwo.size();
  }

  public Integer getDblArrTwo(int index) {
    if (index >= dblArrTwo.size())
      return null;
    return this.dblArrTwo.get(index);
  }

  public Integer setDblArrTwo(int index, Integer value) {
    if (index >= dblArrTwo.size())
      return null;
    return this.dblArrTwo.set(index, value);
  }

  public Integer removeDblArrTwo(int index) {
    if (index >= dblArrTwo.size())
      return null;
    return this.dblArrTwo.remove(index);
  }

  public Integer popFrontDblArrTwo() {
    if (0 == dblArrTwo.size())
      return null;
    return this.dblArrTwo.remove(0);
  }

  public void pushBackDblArrTwo(Integer value) {
...
public:
  std::vector<uint64_t> dblArrTwo;

public:
  std::vector<uint64_t>::size_type sizeDblArrTwo() {
    return this->dblArrTwo.size();
  }

  uint64_t & atDblArrTwo(std::vector<uint64_t>::size_type pos) {
    if (pos >= dblArrTwo.size())
      throw std::invalid_argument("Vector position/index out of bounds!");
    return this->dblArrTwo.at(pos);
  }

  uint64_t popFrontDblArrTwo() {
    if (0 == dblArrTwo.size())
      throw std::length_error("Vector is empty!");
    uint64_t ret = this->dblArrTwo.front();
    this->dblArrTwo.erase(dblArrTwo.begin());
    return ret;
  }

  void pushBackDblArrTwo(uint64_t value) {
    this->dblArrTwo.push_back(value);
  }

  std::vector<uint64_t>::const_iterator iteratorDblArrTwo() {
